...
Iopt <- Ioptimality(H, X.opt.approx, n.bin = (p-1), approx = TRUE)
Iopt

proc.time()-t1

################################################### Inicio de la metaheuristica ###################################################
#Tiempo y fijado de semilla
t <- proc.time()
set.seed(4338663)
...
I_Opt         <- FALSE    #True es para entropia cruzada para I-opt
G_Opt         <- FALSE    #True es para entropia cruzada para G-opt
Panza         <- TRUE    #True es para entropia cruzada para la panza
...
X.E           <- matrix(rep(0, times = N_elite*(n+3)), ncol = N_elite, nrow = (n+3))

if(G_Opt == TRUE | I_Opt == TRUE){
  X.Sol           <- matrix(rep(0, times = (iter + 1)*(n+3)), ncol = (iter + 1), nrow = (n+3))
}else{
  sol             <- matrix(rep(0, times = n+3), ncol = 1, nrow = (n+3))
  X.Sol           <- matrix(rep(0, times = n+3), ncol = 1, nrow = (n+3))
}
...
for(i in 1:N_pob){
  ...
  Por.G <- runif(1, min = 0.4, max = 0.5)
  X.pob[(n + 3),i] <- (Por.G*obj.val + (1 - Por.G)*Iopt)/2
}
...
if(Panza == TRUE){
  aux <- order(X.pob[103,], decreasing = FALSE)
}

#Crear la poblacion elite
for(i in 1:N_elite){
  for(j in 1:(n+3)){
    if(i == 1 & (G_Opt == TRUE | I_Opt == TRUE)){
...
if(Panza == TRUE){
  indicador <- find_nondominated_points(t(X.E[c(n + 1,n + 2),]))
  for(r in 1:N_elite){
    if(indicador[r] == TRUE){
      X.E[n+3,r] <- X.E[n+3,r] + 1
    }
  }
  
  for(i in 1:N_elite){
    if(X.E[n+3, i] >= 1){
      X.E[n+3,i] <- X.E[n+3,i] - 1
      for(k in 1:(n+3)){
        sol[k,1] <- X.E[k,i]
      }
      
      X.Sol <- cbind(X.Sol, sol)
    }
...
for( y in 1:iter ){
  ...
  X.pob.E           <- matrix(rep(0, times = N_new*(n+3)), ncol = N_new, nrow = (n+3))
  ...
  for(i in 1:(N_new)){
    ...
    if(i <= 10){ #Se guardan las primeras 10 soluciones de X.E
      ...
      # Calculo del porcentaje
      Por.G <- runif(1, min = 0.1, max = 0.4)
      X.pob.E[(n + 3),i] <- (Por.G*obj.val + (1 - Por.G)*Iopt)/2
  ...
  if(Panza == TRUE){
    aux <- order(X.pob.E[103,], decreasing = FALSE)
  }
